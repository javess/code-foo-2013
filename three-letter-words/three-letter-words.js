...
function binarySearch(word,dict,lo,hi){
	...
	return binarySearch(word, dict, index, hi);
    }else{
	return binarySearch(word, dict, lo, index);
    }
}
    

//Initialize an adjancency graph to speed up the lookup process
//This is potentially memory intensive and could be improved on by only saving the index
function createGraphStructure(words){
    //Initialize a graph structure for easy navigation
    var graph = [];
    for(var i = 0; i<words.length; i++){
	graph[i] = expandFromWords(words[i]);
    }
    return graph;
}
...
function expandFromWords(word){
	...
	if(words[i] == word)
	    continue;
	c=0;
	//Check for match of at least 2 letters
	c+=words[i][0]==word[0]?1:0;
	c+=words[i][1]==word[1]?1:0;
	c+=words[i][2]==word[2]?1:0;
	if(c == 2){
	    a.push(i);	    
	}
    }
...
//Run a bfs
function breadthFirstSearch(a,b){

    //We basically run a BFS on the graph structure
    var visited = [];
    var graph = createGraphStructure(words);

    //Verify that the initial and final word exist
    var start = binarySearch(a,words,0,words.length);
    var end  = binarySearch(b,words,0,words.length);
    if(start == -1){
	document.write("Word '" +a+ "' in not on the list");
	return null;
    }
    if(end == -1){
	document.write("Word '" +b+ "' in not on the list");
	return null;
    }

    //If initial and final word are equal return basic path
    if(a==b)return [start];

    
    //Initialize BSF stack
    var stack = [];
    stack.push([start]);
    while(stack.length>0 ){
	
	//Get elements as FIFO
	var item = stack.shift();
	
	var expandingNodeIndex = item[item.length-1];
	//Flag node as visited
	visited[expandingNodeIndex] = true;
	//Expand nodes
	var expansion = graph[expandingNodeIndex];
	for(var i = 0; i < expansion.length; i++){
	    if(!visited[expansion[i]]){
		var newPath = item.slice();
		newPath.push(expansion[i]);
		if(expansion[i] == end){
		    return newPath;
		}else{
		    stack.push(newPath);
...
function transform(a,b){
    var path = breadthFirstSearch(a,b);
    document.write("How many moves does it take to change '"+a+"' to '"+b+"'?<br/><br/>");
    if(path == null){
	...
	for(var i = 0; i<path.length; i++){
	    if(i==0){
		document.write(words[path[i]]);
	    }else{
		document.write(" &rarr; "+words[path[i]]);
	    }
	}
    	
	document.write(". "+path.length+" moves");
    }
    document.write("<hr>");
